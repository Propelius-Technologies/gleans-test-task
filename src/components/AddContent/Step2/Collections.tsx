...
const Collections: FC<Props> = ({ onSave, defaultCollections }) => {
  const [selectedCollections, setSelectedCollections] = useState<Collection[]>(defaultCollections)
  ...
  const handleSelectCollection = (collectionToAdd: Collection) => {
    ...
    if (selectedCollections.includes(collectionToAdd)) {
      newCollections = newCollections.filter(collection => collection.id !== collectionToAdd.id)
    } else {
